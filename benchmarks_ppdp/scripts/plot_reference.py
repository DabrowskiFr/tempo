#!/usr/bin/env python3
"""Plot frozen benchmark medians for the selected Tempo/ReactiveML pair."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--out-pdf",
        type=Path,
        default=root / "figures" / "evaluation-time-curves-reference.pdf",
    )
    parser.add_argument(
        "--out-png",
        type=Path,
        default=root / "figures" / "evaluation-time-curves-reference.png",
    )
    parser.add_argument("--bootstrap-samples", type=int, default=20000)
    parser.add_argument("--seed", type=int, default=20260417)
    args = parser.parse_args()

    values = load_values([args.tempo, args.rml], metric="time_ms")
    ...
    for i, bench in enumerate(BENCH_ORDER):
        ...
        for impl, color, marker, label in [
            ...
            points = sorted(
                (size, t)
                for (impl_k, bench_k, size), t in medians.items()
                if impl_k == impl and bench_k == bench
            )
    ...
    ax.set_title("B6 Aggregate speed ratio (B1-B5)", fontsize=11)
    ax.set_xscale("log")
    ax.set_xlabel("size n")
    ...
    fig.suptitle(
        "Tempo vs ReactiveML: execution-time scaling by benchmark and aggregate ratio\n"
