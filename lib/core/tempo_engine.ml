...
let rec run_batch before_step after_step st remaining =
  match remaining with
  | Some n when n <= 0 -> ()
  | _ ->
      let scheduled = run_one_instant before_step after_step st in
      if scheduled then run_batch before_step after_step st (Option.map pred remaining)
      else Tempo_log.log ~level:Logs.Debug (log_ctx st) "tasks" "no more tasks | stop"

let rec run_interactive_loop ~output ~(input : 'input interactive_source) ~wakeup st input_signal output_signal =
  let before_step () =
...
let execute ?instants ?(input = fun () -> None) ?(output = fun _ -> ()) initial =
  ...
  Fun.protect
    ...
    (fun () -> run_batch before_step after_step st instants);
  Tempo_log.log_duration_summary ()
