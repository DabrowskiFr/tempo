...
let make_input_poller source ui () =
  match poll_events source ui with
  | Some _ as batch -> batch
  | None ->
      let now = Unix.gettimeofday () in
...
let () =
  ...
  let rec run_selected () =
    ...
    let input = make_input_poller source ui in
    ui.is_ready <- true;
    ...
    let next_action =
      try
        execute ~input ~output:(render_output score height ui synth bridge)
