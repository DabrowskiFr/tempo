...
let process (bus : Bus.t) (world : world) (student : student) =
  ...
  let caught_cooldown_frames profile difficulty =
    ...
    | _, Chaotic -> 3 * 60
  in
  let base_window_open tick =
    let initial_delay, period, duration =
      match student.profile with
      | Prudent ->
          ( (student.id * 13) mod 280
          , 280
          , int_of_float (72.0 *. world.cheat_window_factor) )
      | Opportunist ->
          ( (student.id * 17) mod 220
          , 220
          , int_of_float (106.0 *. world.cheat_window_factor) )
      | Chaotic ->
          ( (student.id * 37) mod 96
          , 96
          , int_of_float (max 18.0 (30.0 +. ((world.cheat_window_factor -. 1.0) *. 16.0))) )
    in
    if tick < initial_delay
    then false
    else ((tick - initial_delay) mod period) < max 1 duration
  in
    ...
    List.iter
      (function
        | Student_caught id when id = student.id ->
            ...
            student.caught_cooldown <-
              caught_cooldown_frames student.profile world.difficulty;
            student.tell <- 0.0
        | _ -> ())
      events;
    event_process ()
  ...
  let rec behavior_process () =
    ...
    let base = base_window_open tick in
    let cheating_now =
      if world.game_over || not world.started || world.paused then false
        ...
        | Opportunist ->
            ...
            base
            && (not in_range
               || (!opportunist_phase mod 44) < opportunistic_window)
        | Chaotic -> base
    in
    ...
    if cheating_now <> student.cheating then
      Tempo.emit bus.evt
        [ if cheating_now then Cheating_start student.id
          else Cheating_stop student.id
        ];
    student.cheating <- cheating_now;
    opportunist_phase := !opportunist_phase + 1;
    if student.caught_cooldown > 0 then
      student.caught_cooldown <- student.caught_cooldown - 1;
    behavior_process ()
  in
  Tempo.parallel [ event_process; restart_process; behavior_process ]
